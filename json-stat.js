...
JSONstat.version="0.7.3",function(){"use strict"
...
var v=o.dimension,d=v.role||null,p=v.id,g=v.size.length,y=function(t){d.hasOwnProperty(t)||(d[t]=null)}
if(this.length=g,this.id=p,d&&(y("time"),y("geo"),y("metric"),y("classification")),d&&null===d.classification){var _=d.time.concat(d.geo).concat(d.metric),b=function(t,e){for(var n=e.length;n--;)if(t===e[n])return!0
return!1}
d.classification=[]
for(var c=0;g>c;c++)b(p[c],_)||d.classification.push(p[c])
0===d.classification.length&&(d.classification=null)}this.role=d,this.n=u
for(var m=0,w=this.length;w>m;m++)if(v[v.id[m]].category.hasOwnProperty("index")){if(t(v[v.id[m]].category.index)){for(var O={},x=v[v.id[m]].category.index,s=0,S=x.length;S>s;s++)O[x[s]]=s
v[v.id[m]].category.index=O}}else{var P=0
v[v.id[m]].category.index={}
for(var a in v[v.id[m]].category.label)v[v.id[m]].category.index[a]=P++}}else this.length=0
break
case"dimension":var D=[],o=e.__tree__,j=o.category
if(!e.hasOwnProperty("__tree__")||!o.hasOwnProperty("category"))return
if(!j.hasOwnProperty("label")){j.label={}
for(var a in j.index)j.label[a]=a}for(var a in j.index)D[j.index[a]]=a
this.__tree__=o,this.label=o.label||null,this.note=o.note||null,this.link=o.link||null,this.href=o.href||null,this.id=D,this.length=D.length,this.role=e.role,this.hierarchy=j.hasOwnProperty("child"),this.extension=o.extension||null
break
...
for(var f=1,c=0,v=[],d=[],s=[],r=0;h>r;r++)if(void 0!==e[r]){if("number"!=typeof e[r]||e[r]>=u[r])return null
f*=r>0?u[h-r]:1,c+=f*e[h-r-1]}else v.push(r),d.push(u[r])
if(v.length>1)return null
if(1===v.length){for(var p=0,g=d[0];g>p;p++){for(var y=[],r=0;h>r;r++)y.push(r!==v[0]?e[r]:p)
...
void 0!==u&&r.push(u)}return"object"===t.type?{cols:i.cols,rows:r}:("array"===t.type&&r.unshift(i[0]),r)}if("arrobj"===t.type){for(var i=this.toTable({field:"id",content:t.content,status:t.status}),h=[],f=i.shift(),s=0,o=i.length;o>s;s++){for(var c={},v=i[s].length;v--;)c[f[v]]=i[s][v]
h.push(c)}return h}var d="id"===t.field
if("object"===t.type)var p="number"==typeof this.value[0]||null===this.value[0]?"number":"string",g=function(t,e){var n=d&&t||e||t
k.push({id:t,label:n,type:"string"})},y=function(t,e,n){var i=d&&"value"||t||"Value",r=d&&"status"||e||"Status"
...
else var g=function(t,e){var n=d&&t||e||t
k.push(n)},y=function(t,e,n){var i=d&&"value"||t||"Value",r=d&&"status"||e||"Status"
...
g(V,q),P*=x[s],D*=x[s]
for(var C=[],v=0;v<x[s];v++)for(var R in m[w[s]].category.index)if(m[w[s]].category.index[R]===v){var X="id"!==t.content&&m[w[s]].category.label?m[w[s]].category.label[R]:R
