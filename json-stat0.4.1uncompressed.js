...
(function(){
				...
				if (ot.hasOwnProperty("dimension")){
					...
					for(var d=0, len=this.length; d<len; d++){
						if (!(otd[otd.id[d]].category.hasOwnProperty("index"))){
							...
							// That is: we normalize it (instead of defining a function deppending on
	...
	jsonstat.prototype.Dataset=function(ds){
		...
		if(typeof this.__tree__[ds]==="undefined"){
			return null;
		}
		return new jsonstat({"__tree__": this.__tree__[ds], "label" : this.__tree__[ds].label, "type" : "ds"});
	}
	...
	jsonstat.prototype.Data=function(e){
		...
		function dimObj2Array(thisds, obj){
			var a=[];
			for (var d=0, len=thisds.dimension.id.length; d<len; d++){
				a.push(obj[thisds.dimension.id[d]]);
			}
			return a;
		}

		if(this===null || typeof this.__tree__==="undefined"){
			return null;
		}

		if(typeof e==="undefined"){
			return {"value" : this.value, "status": this.status, "label": this.__tree__.label, "length" : this.value.length};
		}

		//Data By Position in original array
		if(typeof e==="number"){
			return {"value" : this.value[e] , "status": getStatus(this,e)};//To do: add more metada...
		}
		...
		if(isArray(e)){
			var n=this.__tree__.dimension.size,
				  dims=n.length,
				  mult=1,
				  m=[],
				  res=0
			...
			for(var i=0; i<dims; i++){
				if(e[i]>=n[i]){
					return {"value" : undefined, "status": undefined};//To do: add more metada...
				}
			}
			...
			return {"value" : this.value[res], "status": getStatus(this,res)};//To do: add more metada...
		}

		var id=dimObj2Array(this.__tree__, e);
		var pos=[], otd=this.__tree__.dimension;
